...
const getDashboardData = async (req, res) => { 
    try {
        ...
        const tasksByPriorities = ["Low", "Medium", "High"];
        ...
        const taskPriorityLevels = tasksByPriorities.reduce((acc, priority) => {
            acc[priority] =
                tasksPriorityLevelsRaw.find((item) => item._id === priority)?.count || 0;
            return acc;
        }
        , {});
